...
@section('panel')
    ...
    <div class="row gy-4 mt-2">
        <div class="col-xxl-3 col-sm-6">
            <x-widget bg="success" icon="las la-ticket-alt" link="{{ route('admin.report.ticket.purchase.history') }}" style="6" title="Sold Tickets" value="{{ $widget['total_sold'] }}" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget bg="info" icon="las la-dollar-sign" link="{{ route('admin.report.ticket.purchase.history') }}" style="6" title="Sold Amount" value="{{ showAmount($widget['sold_amount']) }}" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget bg="primary" icon="las la-money-bill-alt" link="{{ route('admin.report.winning.history') }}" style="6" title="Given Prize" value="{{ showAmount($widget['prize_money']) }}" />
        </div>

        @php
            $profit = $widget['sold_amount'] - $widget['prize_money'];
        @endphp

        <div class="col-xxl-3 col-sm-6">
            @if ($profit < 0)
                <x-widget bg="danger" icon="las la-money-bill-alt" link="{{ route('admin.report.winning.history') }}" style="6" title="Loss" value="{{ showAmount(abs($profit)) }}" />
            @else
                <x-widget bg="success" icon="las la-money-bill-alt" link="{{ route('admin.report.winning.history') }}" style="6" title="Profit" value="{{ showAmount($profit) }}" />
            @endif
        </div>
    </div>
    ...
    <div class="row mt-2 gy-4">
        <div class="col-xxl-6">
            <div class="card box-shadow3 mb-4">
                <div class="card-body">
    ...
    <div class="row mb-none-30 mt-30">
        <div class="col-xl-6 mb-30">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex flex-wrap justify-content-between">
                        <h5 class="card-title">@lang('Ticket Sell Report')</h5>

                        <div id="ticketSoldDatePicker" class="border p-1 cursor-pointer rounded">
                            <i class="la la-calendar"></i>&nbsp;
                            <span></span> <i class="la la-caret-down"></i>
                        </div>
                    </div>

                    <div id="ticketSoldChartArea"></div>
                </div>
            </div>
        </div>
        <div class="col-xl-6 mb-30">
...
@push('script')
    <script>
        ...
        let ticketSoldChart = barChart(
            document.querySelector("#ticketSoldChartArea"),
            @json(__(gs('cur_text'))),
            [{
                name: 'Sold Amount',
                data: []
        ...
        const ticketSoldChartMethod = (startDate, endDate) => {
            ...
            const url = @json(route('admin.chart.ticket.sold'));

            $.get(url, data,
                function(data, status) {
                    if (status == 'success') {
                        ticketSoldChart.updateSeries(data.data);
                        ticketSoldChart.updateOptions({
                            xaxis: {
                                categories: data.created_on,
                            }
                        });
        ...
        $('#ticketSoldDatePicker').daterangepicker(dateRangeOptions, (start, end) => changeDatePickerText('#ticketSoldDatePicker span', start, end));
        ...
        changeDatePickerText('#ticketSoldDatePicker span', start, end);
        changeDatePickerText('#trxDatePicker span', start, end);

        depositWithdrawChart(start, end);
        ticketSoldChartMethod(start, end);
        ...
        $('#ticketSoldDatePicker').on('apply.daterangepicker', (event, picker) => ticketSoldChartMethod(picker.startDate, picker.endDate));
